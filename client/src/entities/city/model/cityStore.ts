...
import type { City, IBaseCityPort, ICityLocationPort, ICityRepo } from '.';

export class CityStore implements IBaseCityPort, ICityLocationPort {
	...
	deleteCity(): void {
		this.clearDebounce();
		this.setReady(createDefaultCity());

		locationChannel.emit({ type: 'location_updated', city: this.name });

		if (this.userStore.id) void this.repo.deleteCity(this.userStore.id);
	}
