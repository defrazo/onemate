...
export class UserProfileStore implements IUserProfileAccountPort, IUserProfileProfilePort, IUserProfileThemePort {
	...
	get birthMonth(): string {
		return this.profile?.birth_date?.split('-')[1] ?? '';
	}

	get birthDay(): string {
		return this.profile?.birth_date?.split('-')[2] ?? '';
	}
	...
	async loadProfile(): Promise<void> {
		...
		try {
			...
			runInAction(() => {
				const merged: UserProfile = { ...createDefaultProfile(), ...server };
				...
				if (cachedUi?.widgets_slots?.length) merged.widgets_slots = cachedUi.widgets_slots;
	...
	async updateProfile(profile: UserProfile): Promise<void> {
		...
		try {
			const updated = await this.repo.updateProfile(this.userStore.id, profile);

			runInAction(() => (this.profile = { ...createDefaultProfile(), ...updated }));
	...
	async updateAvatar(avatar: string): Promise<void> {
		if (!this.userStore.id) return;
		if (this.profile?.avatar_url === avatar) return;

		runInAction(() => {
			if (this.profile) this.profile.avatar_url = avatar;
		});
	...
	async updateWidgetSequence(widgets: string[]): Promise<void> {
		...
		runInAction(() => {
			if (this.profile) this.profile.widgets_sequence = widgets;
		});
	...
	async updateWidgetSlots(slots: string[]): Promise<void> {
		...
		runInAction(() => {
			if (this.profile) this.profile.widgets_slots = slots;
		});
	...
	async markPasswordChanged(id: string): Promise<void> {
		try {
			...
			runInAction(() => {
				...
				this.profile = { ...this.profile, password_changed_at: iso };
			});
		} catch (error) {
			this.setError(error);
		}
	...
	private applyCache(ui: NonNullable<Cache['ui']>): void {
		...
		if (ui.widgets_slots?.length) this.profile.widgets_slots = ui.widgets_slots;
	}
	...
	private cacheSync(): void {
		...
		if (this.profile.widgets_slots) cache.setSlots(this.userStore.id, this.profile.widgets_slots);
	}
	...
	private clearDebounce(): void {
		Object.values(this.debounce).forEach((timer) => {
	...
	private reset(): void {
		...
		if (this.userStore.lastId) {
			storage.remove(key(this.userStore.lastId, 'profile'));
			clearCache(this.userStore.lastId);
		}
	}
