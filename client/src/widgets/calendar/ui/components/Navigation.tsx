...
export const Navigation = ({ month, onNext, onPrev }: NavigationProps) => {
	return (
		<div className="flex h-8 items-center justify-center">
			<Button
				centerIcon={
					<IconChevronLeft className="size-4 text-(--color-secondary) transition-colors group-hover:text-(--accent-default)" />
				}
			...
			<Button
				centerIcon={
					<IconChevronRight className="size-4 text-(--color-secondary) transition-colors group-hover:text-(--accent-default)" />
				}
				className="group size-6 rounded-lg hover:bg-(--accent-default)/10"
				size="custom"
