...
export const SecureTab = observer(() => {
	...
	const handleSave = async () => {
		try {
			await validatePasswords(userStore.passwords[1], userStore.passwords[2]);

			await accountStore.updatePassword(userStore.passwords[0], userStore.passwords[1], userStore.passwords[2]);
		...
		<div className="core-base flex cursor-default flex-col gap-4 rounded-xl pb-4 md:p-4 md:shadow-(--shadow)">
			...
			<div className="flex flex-col gap-2">
				...
				<Input
					...
					variant="ghost"
					onBlur={(e) => userStore.setPasswords(0, e.target.value)}
				...
				<div className="relative">
					<Input
						...
						onBlur={(e) => {
							setShowHint(false);
							userStore.setPasswords(1, e.target.value.trim());
						}}
						onChange={(e) => userStore.setPasswords(1, e.target.value)}
						onFocus={() => setShowHint(true)}
					/>
					<PasswordHint
						password={userStore.passwords[1]}
						showHint={showHint}
				...
				<Input
					...
					value={userStore.passwords[2]}
					variant="ghost"
					onBlur={(e) => userStore.setPasswords(2, e.target.value.trim())}
					onChange={(e) => userStore.setPasswords(2, e.target.value)}
					onPaste={(e) => {
						e.preventDefault();
				...
				<div className="mt-2 flex justify-center gap-2">
					<Button
						...
						disabled={
							!isPasswordValid ||
							!userStore.passwords[0] ||
							!userStore.passwords[1] ||
							!userStore.passwords[2]
						}
					...
					<Button
						disabled={userStore.passwords.every((password) => password === '')}
						variant="warning"
						onClick={() =>
							device === 'mobile'
