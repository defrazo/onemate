...
export class AccountStore {
	...
	async updateEmail(email: string, currentPassword: string): Promise<void> {
		...
		await this.userStore.updateEmail(email, currentPassword);
	}

	async resendPendingEmail(): Promise<void> {
		if (!this.userStore.id) throw new Error('Пользователь не авторизован');
		await this.userStore.resendPendingEmail();
	}

	async cancelPendingEmail(): Promise<void> {
		if (!this.userStore.id) throw new Error('Пользователь не авторизован');
		await this.userStore.cancelPendingEmail();
	}
	...
	async updatePassword(currentPassword: string, password: string, passwordConfirmation: string): Promise<void> {
		if (!this.userStore.id) throw new Error('Пользователь не авторизован');
		await validatePasswords(password, passwordConfirmation);

		await this.userStore.updatePassword(currentPassword, password, passwordConfirmation);
	}
	...
	async restoreAccount(): Promise<void> {
		if (!this.userStore.id) throw new Error('Пользователь не авторизован');
		await this.userStore.restoreAccount();
	}
