...
export const fetchCitiesByName = async (query: string, signal?: AbortSignal): Promise<City[]> => {
	try {
		const url = `${API_URLS.OPENWEATHER_GEOCITY}?q=${encodeURIComponent(query)}&limit=10&appid=${env.OPENWEATHER_API_KEY}&lang=ru`;
		const response = await fetch(url, { signal });

		if (!response.ok) throw new Error('Произошла ошибка при получении списка городов');
